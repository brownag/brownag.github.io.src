...
res <- quantile(sp1$depth, probs = c(0.05,0.5,0.95))
```
...
This small dataset of `r length(sp1)` pedons has a distinctly bimodal appearance -- there appear to be two groups: moderately deep and very deep.
...
There is  value in having a highdetail in descriptions -- but it requires considering properties (like contacts) "in aggregate" after some sort of risk-weighting logic.
...
My preference from here would be to leave `estimateSoilDepth` alone, and generalize `depthOf` further for arbitrary logical expressions (like `aqp::filter`). Pattern matching behavior could be replicated using `grepl` based expressions.

The old dataset that was pulled before cementation and other criteria were included in the default fetchNASIS result. Dylan already did the diagnostic horizon based analysis.

Here we inspect for evidence of pedogenesis. We can clearly see that all Cd horizons without `k` or `y` suffixes (`depth_to_Cd2`) are deeper than the corresponding estimate that looked for any horizon just containing `Cd`: the default part used in `estimateSoilDepth. A custom pattern to filter to Cd without pedogenesis or Cr or R would be `"Cd[^ky]|Cr|R"`
